"""This module implements Independent Q-learning."""
import time
import numpy as np
import torch
from ..env import Environment
from ..utils import ReplayBuffer, save_results
from .agent import LeaderInd, FollowerInd


class IndependentLearn:
    def __init__(self, parameters, env) -> None:
        ...
        self.n_update_q = parameters['update_target_step']
    ...
    def compute_ind(self, ll, ff, s):
        '''
        Compute independent actions from leader and follower's online Q-networks respectively.
        '''
        s = s[None,:].float().to(self.device) if torch.is_tensor(s) else torch.from_numpy(s[None, :]).float().to(self.device)
        with torch.no_grad():
            Ql = ll.onlineQ(s)
            Qf = ff.onlineQ(s)
        al = Ql.argmax().item() - 1     # convert action from index
        af = Qf.argmax().item() - 1
        return al, af
    ...
    def train(self, parameters, env, exp_id=None):
        '''
        Implement independent DQN algorithm.
        '''
        import logging
        logging.basicConfig(format='%(levelname)s: %(message)s')
        logger = logging.getLogger('Ind train')
        logger.setLevel(logging.INFO)

        leader = LeaderInd(parameters, env)
        follower = FollowerInd(parameters, env)
        ...
        #bd_list = env.generate_random_state()               # for random state generation
        ...
        for ep in range(self.n_episode):
            ...
            for t in range(self.n_step_ep):
                al_ind, af_ind = self.compute_ind(leader, follower, s0)
                al = leader.get_action_from_epsgreedy(al_ind)
                af = follower.get_action_from_epsgreedy(af_ind)
                ...
                with torch.no_grad():
                    ql = leader.targetQ(snew_batch).max(dim=1)[0].unsqueeze(-1)
                    qf = follower.targetQ(snew_batch).max(dim=1)[0].unsqueeze(-1)
                done = 1 * torch.all(snew_batch == 0, dim=1).unsqueeze(1)
                yl = rl_batch + leader.gam * (1-done) * ql
                yf = rf_batch + follower.gam * (1-done) * qf
                loss_l = loss_fn(leader.onlineQ(s_batch).gather(1, idx_al), yl)
                loss_f = loss_fn(follower.onlineQ(s_batch).gather(1, idx_af), yf)     
                ...
                # hard update target network (tau=1)
                cnt += 1
                if cnt % self.n_update_q == 0:
            ...
            logger.info(f'time for Episode {ep+1}: {100*(time.time()-st):.3f} ms.')
        ...
        if save_flag:
            ...
            dname = os.path.join(train_data_dir, 'ind', f'task{parameters["task_id"]}')
    ...
    def eval_policy(self, p, leader, follower):
        ...
        for t in range(self.n_step_ep):
            al, af = self.compute_ind(leader, follower, s0)
