...
from sg_taskplan.other.nash import NashLearn
...
def train(task_id=1):
    ...
    nashtrainner = NashLearn(param, env)
    nashtrainner.train(param, env, exp_id=None)
...
def train_multiproc(task_id=1):
    ...
    device_list = ['cuda:0']*0 + ['cuda:1']*0 + ['cuda:2']*0 + ['cuda:3']*0 + ['cpu']*10
    arg_list = []
...
def run_train_wrapper(param, env, exp_id):
    ...
    nashtrainner = NashLearn(param, env)
    return nashtrainner.train(param, env, exp_id)
