...
from sg_taskplan.other.independent import IndependentLearn
...
def train(task_id=1):
    ...
    indtrainner = IndependentLearn(param, env)
    indtrainner.train(param, env, exp_id=None)
...
def train_multiproc(task_id=1):
    ...
    device_list = ['cuda:0']*2 + ['cuda:1']*3 + ['cuda:2']*4 + ['cuda:3']*0 + ['cpu']*10
    arg_list = []
...
def run_train_wrapper(param, env, exp_id):
    ...
    indtrainner = IndependentLearn(param, env)
    return indtrainner.train(param, env, exp_id)
